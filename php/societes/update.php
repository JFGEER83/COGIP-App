<?php require 'database.php'; 
             $ids = null; if ( !empty($_GET['idSOCIETES'])) 
             { 
                 $ids = $_REQUEST['idSOCIETES']; 
                } 
                if ( null==$ids ) 
                { 
                ...
                {  
                    ...
                    $pays = $_POST['pays_societe']; 
                    $tva = $_POST['TVA_societe']; 
                    $tel = $_POST['telephone_societe']; 
                    $id = $_POST['TYPE_idTYPE']; 
                     
                    
                    $valid = true; if (empty($societe)) 
                    { 
                        $societeError = 'SVP enter un nom de société'; 
                        $valid = false; 
                    } 
                    if (empty($adresse)) 
                    { 
                        $adresseeError = 'SVP enter une adresse'; 
                        $valid = false; 
                    } 
                    if (empty($pays)) 
                    { 
                        $paysError = 'SVP entrer un pays'; 
                        $valid = false; 
                    } 
                    if (empty($tel)) 
                    { 
                        $telError = 'SVP entrer un numéro de téléphone'; 
                        $valid = false; 
                    } 
                    if (empty($tel)) 
                    { 
             ...
             $sql = "UPDATE SOCIETES SET name = ?,nom_societe = ?,adresse_societe = ?,pays_societe = ?, TVA_societe = ?, telephone_societe = ?, TYPE_idTYPE = ?, WHERE idSOCIETES = ?";
             $q = $pdo->prepare($sql);
             $q->execute(array($societe,$adresse, $pays, $tva, $tel,$pays,$id));
             Database::disconnect();
             header("Location: index.php");
         } 
        else 
        {
             $pdo = Database::connect();
             $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
             $sql = "SELECT * FROM SOCIETE where idSOCIETES = ?";
             $q = $pdo->prepare($sql);
             $q->execute(array($id));
             $data = $q->fetch(PDO::FETCH_ASSOC);
             $societe = $data['nom_societe'];
             $adresse = $data['adresse_societe'];
             $pays = $data['pays_societe'];
             $tva = $data['TVA_societe'];
             $tel = $data['telephone_societe'];
             $id = $data['TYPE_idTYPE'];
             Database::disconnect();
         }
